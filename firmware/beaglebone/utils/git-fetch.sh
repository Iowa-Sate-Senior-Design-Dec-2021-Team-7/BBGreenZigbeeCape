...
# clones desired repo and and checkouts branch if dir doesn't exist, or checkout
#	branch and pull to update based on configuration defined below
...
git_user_token=-jCg5yGt2DPnWcKjiyBe                 # pwd/token to verify access with
git_remote_filter=firmware/beaglebone               # files to check out from repo
git_local_dir_abs=/var/lib/cloud9/sddec          	# local directory repo is located at (absolute path)
